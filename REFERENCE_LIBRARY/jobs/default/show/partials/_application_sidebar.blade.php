...
<div class="apply-sidebar sticky-top" style="top: 20px;">
    ...
    <div class="card glass-surface p-4 mb-3 text-center">
        <h5 class="fw-bold mb-3"><i class="bi bi-building-fill me-2 text-primary-color"></i>Company Snapshot</h5>
        ...
        <p class="small text-muted mb-0">Based on {{ number_format($reviewCount) }} employee reviews</p>
        <a href="{{ route('partner.profile', $job->employer) }}" class="small text-primary-color text-decoration-none mt-2 fw-semibold">View Company Profile</a>
    </div>
    ...
    <div class="card glass-surface p-4 mb-3">
        ...
        <div class="mb-4 text-center">
            <p class="mb-1 fw-semibold">Application Deadline:</p>
            @php
            ...
            <span class="badge text-white fw-bold p-2 fs-6 {{ $badgeClass }}">
                {{ $isExpired ? 'EXPIRED' : $deadline->format('F d, Y') }}
            </span>
            @if (!$isExpired)
                <p class="small text-muted mt-1 mb-0">Time remaining: {{ $deadline->diffForHumans(null, true) }}</p>
            @endif
        </div>
        ...
        <div class="d-grid mb-3">
            ...
            <a href="{{ route('jobs.apply', $job->slug) }}" class="btn btn-lg fw-bold text-white btn-primary {{ $isExpired ? 'disabled' : '' }}">
                <i class="bi bi-box-arrow-in-right me-2"></i>Apply Now
            </a>
        </div>
        <div class="d-grid mb-4">
            {{-- Placeholder for external application link --}}
            <button class="btn btn-lg btn-outline-secondary fw-bold" {{ $isExpired ? 'disabled' : '' }}><i class="bi bi-linkedin me-2"></i>Apply with LinkedIn</button>
        </div>
        ...
        <div class="text-center small text-muted">
            ...
            <button 
                ...
                data-job-id="{{ $job->id }}">
                <i class="bi bi-heart me-1"></i>Save This Job
            </button> 
            ...
            <a href="{{ route('partner.profile', $job->employer) }}" class="btn btn-link p-0 text-primary-color text-decoration-none fw-semibold">
                <i class="bi bi-person-add me-1"></i>Follow {{ $companyName }}
            </a>
        </div>
    </div>
    ...
    <div class="card glass-surface p-4 mb-3">
        <h6 class="fw-bold mb-3"><i class="bi bi-fast-forward me-2 text-primary-color"></i>Hiring Process</h6>
    ...
    <div class="card glass-surface p-3 text-center">
        <h6 class="fw-bold mb-2">Questions? Contact HR:</h6>
