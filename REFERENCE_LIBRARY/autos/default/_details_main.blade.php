<div class="card glass-surface p-0 border-0 shadow-lg overflow-hidden">
    {{-- Gallery Integration --}}
    @include('frontend.themes.autos.default.show.partials._gallery')
    ...
    <div class="p-4 p-lg-5">
        ...
        {{-- Dealer Description --}}
        <h4 class="fw-bold mb-3 text-dark">Dealer Comments</h4>
        <div class="listing-description">
            {!! nl2br(e($auto->description)) !!}
        </div>

        {{-- Tech Specs Table --}}
        <h4 class="fw-bold mt-5 mb-4 text-dark"><i class="bi bi-info-circle-fill me-2 text-primary-color"></i>Technical Specifications</h4>
        <div class="row g-0">
            <div class="col-12">
                <table class="table table-borderless spec-table mb-0">
                    <tbody>
                        <tr>
                            <th>Condition</th><td>{{ $auto->condition_rating }}/10</td>
                            <th>Drivetrain</th><td>{{ $auto->drivetrain ?? 'N/A' }}</td>
                        </tr>
                        <tr>
                            <th>Fuel Economy</th><td>{{ $auto->fuel_economy ?? 'N/A' }}</td>
                            <th>VIN</th><td class="small font-monospace">{{ $auto->vin_number ?? 'Request via Dealer' }}</td>
                        </tr>
                        <tr>
                            <th>Warranty</th><td>{{ $auto->warranty_months ? $auto->warranty_months . ' Months' : 'As-Is' }}</td>
                            <th>Stock #</th><td>{{ $auto->stock_quantity ?? '1' }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div class="row g-3 mt-4">
            <div class="col-md-4 text-center">
                <div class="p-3 bg-light rounded-4 border border-white h-100">
                    ...
                    <span class="fw-bold small d-block">Quality Verified</span>
                    <span class="text-muted smaller">150-Point Inspection</span>
                </div>
            </div>
            <div class="col-md-4 text-center">
                <div class="p-3 bg-light rounded-4 border border-white h-100">
                    <i class="bi bi-file-earmark-medical text-primary-color fs-2 mb-2 d-block"></i>
                    <span class="fw-bold small d-block">CARFAX Report</span>
                    <a href="#" class="text-primary-color smaller text-decoration-none">View History</a>
                </div>
            </div>
            <div class="col-md-4 text-center">
                <div class="p-3 bg-light rounded-4 border border-white h-100">
        ...
        {{-- Features Grid --}}
        <h4 class="fw-bold mt-5 mb-3 text-dark"><i class="bi bi-list-check me-2 text-primary-color"></i>Key Options & Features</h4>
        <div class="row g-3">
            ...
            @empty
                <div class="col-12 text-muted">Standard equipment for this model.</div>
            @endforelse
        </div>
