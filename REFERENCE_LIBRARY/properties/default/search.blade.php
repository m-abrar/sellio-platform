        ...
        <aside class="col-12 col-lg-3">
            <div class="offcanvas-lg offcanvas-start rounded-4 border-0 shadow" tabindex="-1" id="filterSidebar">
                ...
                <div class="offcanvas-body p-0">
                    @include('frontend.themes.properties.default._partials._sidebar_filter')
                </div>
            </div>
        </aside>
        ...
        <div class="col-12 col-lg-9">
            <div class="row g-3 row-cols-2 row-cols-md-2 row-cols-xl-3">
                @forelse($properties as $property)
                    <div class="col">
                        @include('frontend.themes.properties.default._partials._card', ['property' => $property])
                    </div>
                @empty
            ...
            @if($properties->hasPages())
                <div class="mt-5 d-flex justify-content-center">
                    {{ $properties->appends(request()->query())->links('frontend.themes.properties.default._partials._pagination') }}
                </div>
            @endif
        </div>
    </div>
