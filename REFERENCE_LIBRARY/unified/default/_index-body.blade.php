            ...
            @isset($propertiesFeatured)
                @forelse($propertiesFeatured->take(3) as $property)
                    <div class="col" data-aos="fade-up" data-aos-delay="{{ $loop->iteration * 100 }}">
                        @include('frontend.themes.unifieds.default._partials._property-card', ['property' => $property])
                    </div>
                @empty
        ...
        <div class="row row-cols-1 row-cols-sm-2 row-cols-lg-4 g-4">
            @isset($autosLatest)
                @forelse($autosLatest->take(4) as $auto)
                    <div class="col" data-aos="fade-up" data-aos-delay="{{ $loop->iteration * 100 }}">
                        @include('frontend.themes.unifieds.default._partials._auto-card', ['auto' => $auto])
                    </div>
                @empty
                    ...
                    <div class="list-group list-group-flush bg-transparent">
                        @isset($jobsFeatured)
                            @foreach($jobsFeatured->take(4) as $job)
                                @include('frontend.themes.unifieds.default._partials._job-list-item', ['job' => $job])
                            @endforeach
                        @endisset
                    </div>
                </div>
                        ...
                        @isset($classifiedsFeatured)
                            @foreach($classifiedsFeatured->take(4) as $item)
                                <div class="col-6" data-aos="zoom-in" data-aos-delay="{{ $loop->iteration * 100 }}">
                                    @include('frontend.themes.unifieds.default._partials._classified-mini-card', ['item' => $item])
                                </div>
                            @endforeach
                        @endisset
                    </div>
            ...
            @isset($eventsFeatured)
                @forelse($eventsFeatured->take(4) as $event)
                    <div class="col" data-aos="zoom-in" data-aos-delay="{{ $loop->iteration * 100 }}">
                        @include('frontend.themes.unifieds.default._partials._event-card', ['event' => $event])
                    </div>
                @empty
...
{{-- MODULE: CTA --}}
@include('frontend.themes.unifieds.default._partials._index-cta')