        ...
        <aside class="col-12 col-lg-3">
            <div class="offcanvas-lg offcanvas-start rounded-4 border-0 shadow" tabindex="-1" id="filterSidebar">
                ...
                <div class="offcanvas-body p-0">
                    @include('frontend.themes.events.default._partials._sidebar-filter-events')
                </div>
            </div>
        </aside>
        ...
        <div class="col-12 col-lg-9">
            <div class="row g-3 row-cols-2 row-cols-md-2 row-cols-xl-3">
                @forelse($events as $event)
                    <div class="col">
                        @include('frontend.themes.events.default._partials._card-event', ['event' => $event])
                    </div>
                @empty
            ...
            @if($events->hasPages())
                <div class="mt-5 d-flex justify-content-center">
                    {{ $events->links('frontend.themes.events.default._partials._pagination-links') }}
                </div>
            @endif
        </div>
    </div>
