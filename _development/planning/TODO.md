...
- [x] For the installation wizard specifically, switch to a more premium/luxury palette instead of the teal brand colors:
  - Midnight Blue (deep premium base): #0a1c33
  - Classic Champagne Gold (luxury accents): #d1a84d
  - Rich Ochre (secondary typography/icons): #9e7931
  - Deep Ocean Gradient (secondary actions): #1b4363
  (Scoped to `apps/backend/public/install/style.css` only — landing/docs keep the teal brand. `--primary`/`--primary-dark`/`--primary-light`/soft/glow now map to gold/ochre; `--secondary-color` (Deep Ocean) restyles the outline "Back" button; `--background-dark` (Midnight Blue) became the actual page backdrop behind the white glass card, replacing the old light-gray gradient — a real dark/luxury reskin, not just a token swap. Added `--on-primary: var(--background-dark)` and used it for text/icons on gold buttons and the active step icon instead of white — white-on-gold was ~2.2:1 contrast (fails WCAG), navy-on-gold is ~7.7:1. Also fixed the footer links/copyright, which sit directly on the new dark backdrop outside the card, from the light-theme `--text-muted` gray (now ~3.2:1, too low against navy) to translucent white. Completed steps keep the green success checkmarks unchanged (semantic, not brand). Visually verified via Playwright screenshots of the welcome and admin-account steps.)

-------------------

- [x] For the introduction page use these colors:
  - Charcoal Slate (Text/Dark Mode): #3a3e47
  - Vibrant Mint/Green (Growth/Success): #5cb874
  - Soft Sage (Background Accents): #bfe3cf
  - Coral Orange (High-contrast Alerts/Buttons): #f07c52
  (Scoped to `introduction/` — `introduction/css/style.css`, `introduction/product-tour/css/product-tour.css`, the listing-description partials, `index.php`, `config.php`. Coral Orange replaced the teal as `--primary-sellio`/`--primary-dark` (buttons, nav active state, hero accent text, badges); the old `rgba(*, 0.X)` soft/glow tints now use Soft Sage instead of a low-alpha primary, per the explicit "Background Accents" role. `--text-main` and the `[data-theme="dark"]`/`.bg-dark-executive`/`.bg-demos` dark-section surfaces moved from navy to Charcoal Slate shades. Vibrant Mint is set as `--secondary-color`/`--accent-color`, available as a token but not force-applied anywhere specific yet — no obvious "growth/success" stat element to attach it to without inventing one. Follow-up pass: a full-page visual audit turned up one real leftover the earlier hex-signature search couldn't catch — `.flipbook-invite-cta` ("Explore the interactive product tour" button) had a hardcoded `#86d64d` green gradient stop, a shade close to but not matching the old brand green's exact hex, so it survived both rebrand passes. Swapped it for a light-coral stop (`#ffb199`) to pair with the primary. Audited every remaining hardcoded hex across `introduction/`, `documentation/`, and the install wizard for similar near-miss colors — everything else left in place is either a deliberate per-module/per-technology badge color (`config.php`'s 10 industry colors, `tech-stack.php`'s per-language icon colors), a semantic status color (success/warning/danger/info), or a decorative macOS-traffic-light dot, none of which are brand tokens.)

- [x] For the documentation page use these colors:
  - Royal Indigo (Primary Branding): #4a2c85
  - Electric Purple (Secondary/Badges): #8b3cc4
  - Deep Plum (Alternative Background): #413247
  - Sunset Orange (Primary CTA/Attention): #f46136
  
  (Scoped to `documentation/assets/css/style.css`, `documentation/index.html`, `documentation/reviewer/index.html`. Split the two "primary" roles the palette implies: Sunset Orange became `--primary`/`--primary-hover` (buttons, active nav item, links, hero accent word — matches "CTA/Attention"), while Royal Indigo (`--secondary-color`) was applied specifically to the sidebar wordmark (`.logo-text`) and logo-mark fallback gradient for "Primary Branding" identity, kept visually distinct from the interactive orange. Electric Purple (`--accent-color`) restyles `.version-badge` per "Secondary/Badges". Deep Plum (`--background-dark`) replaced the navy `--code-bg` for code blocks, matching "Alternative Background".)

-----------------------

- [x] In documentation, too much use of color Sunset Orange — only use for 10-15%, use Royal Indigo and Deep Plum and Electric Purple as primary and secondary and other support colors. (Reassigned `documentation/assets/css/style.css` tokens: `--primary`/`--primary-hover`/soft/glow flipped from Sunset Orange to Royal Indigo (buttons, active nav, links, hero accent word — now the dominant color). `--secondary-color` moved from Indigo to Electric Purple, used for `.logo-mark`/`.logo-text` (brand wordmark). `--background-dark`/`--code-bg` stayed Deep Plum. `--accent-color` moved from Purple to Sunset Orange, now confined to just `.version-badge`'s text — the one remaining orange element on the page.)

- [x] In the landing/introduction page, too much use of Coral Orange — only use for 10-15%, use Vibrant Mint/Green as primary. (`introduction/css/style.css`, `product-tour/css/product-tour.css`, the listing-description partials, `index.php`, `config.php`: `--primary-sellio`/`--primary-dark` flipped from Coral to Vibrant Mint — this drives the vast majority of colored elements (`.bg-sellio-solid` alone covers 17+ badges/progress-bars/chart-bars/avatars/buttons across the page). `--secondary-color`/`--accent-color` moved to Coral Orange, and a new `.bg-accent-solid` class was applied to exactly one element — the hero "PRODUCTION READY v2.4.0" badge — as the sole remaining orange accent. NOTE: superseded below — see next entry reverting introduction's primary back to green.)

- [x] In the installation wizard, stop using green for success/completed status — stick to the given gold/navy/ochre/deep-ocean color set. (`apps/backend/public/install/style.css`: `--success` changed from emerald `#10b981` to Deep Ocean `#1b4363` — re-colors the stepper's completed-step checkmarks/labels, `.info-panel-success`, `.status-banner-success`, and `.success-icon-wrap`. Also converted the hardcoded darker-green gradient stop (`#059669`→`#16364f`), dark-green text (`#047857`→`#1b4363`), and every `rgba(16, 185, 129, *)` soft background to Deep Ocean equivalents. Left `.terminal-dot-green` (decorative macOS-style traffic-light dot) alone — that's a UI convention, not a status color.)

-----------------

- [x] For the documentation, let's go back to `--primary-sellio: #76c043; --primary-dark: #5ba32d;` but add some CTA color too, for buttons like "Get License", do not use CTA more than 5-10%. (`--primary-sellio` names it as introduction's variable and "Get License" only exists in `introduction/navbar.php`, so this was applied to the introduction page, not documentation — reverted `introduction/css/style.css`, `product-tour/css/product-tour.css`, the listing-description partials, `index.php`, and `config.php` from Vibrant Mint back to the original green. Decoupled `.btn-sellio-solid` — used by exactly 3 elements sitewide: "Get License" (`navbar.php`), "Get Started" (`floating-buy-bar.php`), and the exit-intent popup CTA (`popup-exit-intent.php`) — from `var(--primary-sellio)` to `var(--accent-color)` (Coral Orange), with a `filter: brightness(0.9)` hover instead of the old primary-dark hover swap. Coral now appears only on those 3 CTA buttons plus the pre-existing "PRODUCTION READY" badge — everything else (nav pill, active nav link, hero accent text, "Explore the Dashboards" gradient, live-demo badge, hero glow) is back to green.)
