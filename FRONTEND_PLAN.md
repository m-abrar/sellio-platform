...
| ~~17~~ | ~~Cart + Checkout: header treatment + Stripe Elements styling~~ | ~~`products/cart.blade.php`, `_checkout-header.blade.php`, `success.blade.php`, `_checkout_payment_scripts.blade.php`, `style.css`~~ | ✅ Done — dark `.booking-hero` strip on all 3 steps; success page rebuilt with dark hero + success icon + payment reference pill + stepper at step 3; Stripe Elements: `iconStyle:'solid'`, brand text colour `#1C1917`, `stripe-focused` class on focus/blur → orange focus ring in CSS |
