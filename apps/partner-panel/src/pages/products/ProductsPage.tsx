    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      >
        <button 
          ...
          className="bg-slate-900 text-white px-8 py-4.5 rounded-[1.8rem] font-black text-[11px] uppercase tracking-[0.2em] shadow-xl hover:bg-[#6610f2] transition-all active:scale-95 flex items-center gap-2"
        >
      ...
      {isLoading ? (
        <div className="space-y-6">
          ...
          <div className="hidden lg:block bg-white rounded-[2.5rem] border border-slate-100 h-[600px] overflow-hidden p-10">
             <div className="w-full h-full bg-slate-50/50 rounded-[1.5rem] animate-pulse flex items-center justify-center">
          ...
          <div className="lg:hidden space-y-6">
            {products.length === 0 ? (
              ...
              products.map((product) => (
                <div key={product.id} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm relative overflow-hidden group">
                  <div className="flex gap-6">
                    ...
                    <div className="min-w-0 flex-1 pt-1">
                      ...
                      <h3 className="text-lg font-black text-slate-900 truncate mt-2 italic tracking-tight">{product.title}</h3>
                  ...
                  <div className="flex items-center justify-between mt-6 pt-6 border-t border-slate-50">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => navigate(`/dashboard/products/edit/${product.slug}`)} className="p-4 bg-slate-50 text-slate-400 rounded-2xl"><HiOutlinePencilSquare className="w-5 h-5" /></button>
                      <button onClick={() => handleDelete(product.id, product.title)} className="p-4 bg-red-50/50 text-red-400 rounded-2xl"><HiOutlineTrash className="w-5 h-5" /></button>
                    </div>
                  </div>
                </div>
              ))
            ...
            <table className="w-full border-separate border-spacing-0">
              <thead>
                <tr className="text-left bg-slate-50/40">
                  <th className="px-10 py-8 text-[11px] font-black uppercase tracking-[0.3em] text-slate-400">Asset Identity</th>
                      ...
                      <td className="px-10 py-6 text-left">
                        <div className="flex items-center gap-6">
                          ...
                          <div className="min-w-0">
                            <p className="text-lg font-black tracking-tighter mb-1 truncate text-slate-900">{product.title}</p>
                      ...
                      <td className="px-10 py-6 text-right relative">
                        <div className="relative h-16 flex items-center justify-end">
                          ...
                          <div className="absolute inset-y-0 right-0 flex items-center gap-3 opacity-0 translate-y-[-20px] group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500">
                            <button onClick={() => navigate(`/dashboard/products/edit/${product.slug}`)} className="p-4 text-slate-400 bg-white rounded-2xl border border-slate-100 hover:text-[#6610f2] hover:shadow-xl transition-all"><HiOutlinePencilSquare className="w-5 h-5" /></button>
                            <button onClick={() => handleDelete(product.id, product.title)} className="p-4 text-slate-400 bg-white rounded-2xl border border-slate-100 hover:text-red-500 hover:shadow-xl transition-all"><HiOutlineTrash className="w-5 h-5" /></button>
                          </div>
                        </div>
                      </td>
                    </tr>
