...
export default function Sidebar({ user }: any) {
  ...
  const modules = [
    { name: 'Properties', slug: 'properties', icon: HiOutlineHome, type: 'Bookings', count: user?.properties_bookings_count || 0, color: 'bg-blue-50 text-blue-600' },
    { name: 'Events', slug: 'events', icon: HiOutlineCalendar, type: 'Bookings', count: user?.events_bookings_count || 0, color: 'bg-purple-50 text-purple-600' },
    { name: 'Autos', slug: 'autos', icon: HiOutlineTruck, type: 'Inquiries', count: user?.autos_inquiries_count || 0, color: 'bg-orange-50 text-orange-600' },
    { name: 'Jobs', slug: 'joblistings', icon: HiOutlineBriefcase, type: 'Applications', count: user?.job_applications_count || 0, color: 'bg-emerald-50 text-emerald-600' },
    { name: 'Services', slug: 'services', icon: HiOutlineWrenchScrewdriver, type: 'Inquiries', count: user?.services_inquiries_count || 0, color: 'bg-pink-50 text-pink-600' },
    { name: 'Products', slug: 'products', icon: HiOutlineShoppingBag, type: 'Orders', count: user?.orders_count || 0, color: 'bg-indigo-50 text-indigo-600' },
  ];

  const backOfficeLinks = [
    { label: 'Customers', to: '/dashboard/customers', icon: HiOutlineUsers },
    { label: 'Reviews', to: '/dashboard/reviews', icon: HiOutlineStar },
    { label: 'Messages', to: '/dashboard/messages', icon: HiOutlineChatBubbleLeftRight },
    { label: 'Wallet', to: '/dashboard/wallet', icon: HiOutlineWallet },
    { label: 'Payouts', to: '/dashboard/payouts', icon: HiOutlineBanknotes },
    { label: 'Memberships', to: '/dashboard/memberships', icon: HiOutlineShieldCheck },
    ...
    <>
          ...
          <div className="flex-1 overflow-y-auto px-6 pb-40 custom-scrollbar">
            ...
            <div className="grid grid-cols-1 gap-2 mb-6">
              {Array.from(new Set(modules.map(m => m.slug))).map((slug) => (
                <NavLink key={slug} to={`/dashboard/${slug}`} className="flex items-center p-4 bg-white rounded-2xl border border-slate-100 text-slate-600 font-bold text-sm active:bg-slate-50">
            ...
            <div className="grid grid-cols-1 gap-2">
              {backOfficeLinks.map((item, idx) => (
                <NavLink key={idx} to={item.to} className="flex items-center p-4 bg-white rounded-2xl border border-slate-100 text-slate-600 font-bold text-sm active:bg-slate-50">
      ...
      <aside className="hidden lg:flex flex-col sticky top-6 ml-6 h-[94dvh] w-[320px] bg-white border border-slate-100 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.04)] overflow-hidden">
        ...
        <nav className="flex-1 px-6 overflow-y-auto custom-scrollbar pb-10">
          ...
          <div className="space-y-1.5 mb-8">
            {modules.map((mod, i) => (
              <NavLink key={i} to={`/dashboard/${mod.slug}/${mod.type.toLowerCase()}`} className={({ isActive }) => `flex items-center justify-between py-3 px-6 rounded-xl text-[14px] font-bold transition-all ${isActive ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-500 hover:text-slate-900'}`}>
                <div className="flex items-center"><mod.icon className="w-5 h-5 mr-3 opacity-70" /> {mod.name}</div>
          ...
          <div className="space-y-1.5 mb-8">
            {Array.from(new Set(modules.map(m => m.slug))).map((slug) => (
              <NavLink key={slug} to={`/dashboard/${slug}`} className={({ isActive }) => `flex items-center py-3 px-6 rounded-xl text-[14px] font-bold transition-all ${isActive ? 'text-slate-900 bg-slate-50' : 'text-slate-500 hover:text-slate-900'}`}>
          ...
          <div className="space-y-1.5 mb-8">
            {backOfficeLinks.slice(0, 4).map((link, i) => (
              <NavLink key={i} to={link.to} className={({ isActive }) => `flex items-center py-3 px-6 rounded-xl text-[14px] font-bold transition-all ${isActive ? 'text-slate-900 bg-slate-50' : 'text-slate-500 hover:text-slate-900'}`}>
