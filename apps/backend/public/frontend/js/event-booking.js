document.addEventListener('DOMContentLoaded', function() {
    ...
    function generateTicketRows(inventory) {
        ...
        return inventory.map(function(item) {
            ...
            return '<div class="d-flex justify-content-between align-items-center mb-3 pb-3 border-bottom ticket-row"' +
                   ' data-price="' + price + '" data-available="' + available + '">' +
                   '<div class="form-check d-flex align-items-start w-75">' +
                   '<input class="form-check-input mt-2 ticket-type-radio me-3" type="radio"' +
                   ' name="selected_ticket_id" id="type-' + id + '" value="' + id + '">' +
                   '<label class="form-check-label w-100" for="type-' + id + '">' +
                   '<span class="fw-bold mb-0 d-block">' + name + '</span>' +
                   '<span class="fw-bolder text-primary-color price-highlight bg-primary-light px-2 py-1 rounded me-1 fs-5">$' + price.toFixed(2) + '</span>' +
                   '<span class="small text-muted">(' + available + ' left)</span>' +
                   '</label></div>' +
                   '<input type="number" name="quantity" data-ticket-id="' + id + '"' +
                   ' class="form-control form-control-sm text-center quantity-input"' +
                   ' value="1" min="1" max="' + available + '" style="width: 70px;" disabled>' +
                   '</div>';
        }).join('');
    }
    ...
    function calculateTotal() {
        ...
        quantityInputs.forEach(function(input) { input.disabled = true; });
