...
<form id="payment-form" method="POST" action="{{ route('events.tickets.booking.processPayment', ['event' => $booking->event->slug, 'booking' => $booking->id]) }}" data-event-payment-form>
    @csrf
    ...
    <div class="form-check border p-3 mb-2 rounded">
        ...
        <div class="small text-muted mt-1">
            ...
            @else
                Stripe must be configured in admin settings before event ticket payment can be collected.
            @endif
        </div>
    </div>

    <div class="form-check border p-3 mb-2 rounded opacity-75">
        ...
        <label class="form-check-label fw-bold" for="payment_paypal">
            PayPal <span class="badge bg-light text-muted border ms-1">Coming soon</span>
        </label>
        <div class="small text-muted mt-1">PayPal ticket checkout will follow after Stripe is finalized.</div>
    </div>
    
    <div id="stripe-payment-fields" class="mt-4 p-3 border rounded bg-light">
        @if($usesStripeElements)
            <label for="event-cardholder-name" class="form-label small fw-semibold">Cardholder Name</label>
            <input
            ...
            <label for="event-stripe-card-element" class="form-label small fw-semibold">Card Details</label>
            <div
        ...
        @else
            <p class="text-muted small mb-0">Stripe card fields will appear here after the gateway publishable key is configured.</p>
        @endif
    </div>
    
    <hr class="my-4">

    <button type="submit" class="btn btn-success btn-lg w-100" data-event-payment-submit @disabled(!$usesStripeElements)>
