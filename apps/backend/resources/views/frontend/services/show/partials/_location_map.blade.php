...
<div class="mt-5">
    <div class="d-flex align-items-start mb-4">
        <div class="icon-box-sm bg-primary-light text-primary-color me-3 mt-1">
    ...
    <div class="map-wrapper position-relative mb-4">
        <div class="map-container glass-surface d-flex align-items-center justify-content-center overflow-hidden rounded-4 border-0 shadow-sm" 
             style="height: 250px; background: linear-gradient(135deg, #e9ecef 0%, #dee2e6 100%);">
            ...
            <div class="text-center z-1">
                <div class="bg-white rounded-circle shadow-sm d-inline-flex align-items-center justify-content-center mb-2" style="width: 60px; height: 60px;">
                    <i class="bi bi-map text-primary-color fs-3"></i>
                </div>
        ...
        @if (!($isAddressOnly ?? false) && $service?->address)
            <a href="https://www.google.com/maps/search/?api=1&query={{ urlencode($service->address . ' ' . $service->city) }}" 
               target="_blank" 
               class="btn btn-white btn-sm position-absolute bottom-0 end-0 m-3 shadow-sm rounded-pill px-3 fw-bold">
    ...
    <div class="d-flex align-items-center gap-3">
        @if (!($isAddressOnly ?? false) && $service?->address)
            <a href="https://www.google.com/maps/dir/?api=1&destination={{ urlencode($service->address . ' ' . $service->city) }}" 
               target="_blank" 
               class="btn btn-outline-primary-color fw-bold rounded-pill px-4">
               <i class="bi bi-cursor-fill me-2"></i>{{ __('Get Directions') }}
            </a>
        @endif

        @if (($isOffice ?? false) && ($service->phone ?? $service->user?->phone))
            <a href="tel:{{ $service->phone ?? $service->user->phone }}" class="btn btn-link text-primary-color text-decoration-none fw-bold small p-0">
