<a href="{{ route('classifieds.show', $classified->slug) }}"
   class="listing-card h-100 text-decoration-none text-dark d-flex flex-column transition-all rounded-4">

    <div class="img-container position-relative overflow-hidden rounded-top-4">
        <div class="listing-card-img aspect-ratio-4-3">
            <img src="{{ $classified->primary_image_url }}"
    ...
    <div class="card-body p-3 d-flex flex-column flex-grow-1">
        <div class="mb-2">
            ...
            <h6 class="property-title fw-800 mb-1 text-dark text-truncate mt-1">{{ $classified->title }}</h6>
        </div>
        ...
        <div class="d-flex justify-content-between align-items-center pt-3 border-top mt-3">
            <span class="small text-muted">
                <i class="bi bi-clock me-1"></i>{{ $classified->created_at->diffForHumans() }}
            </span>
            <span class="btn-primary-light btn-sm rounded-2 fw-semibold">{{ __('View details') }}</span>
        </div>
    </div>
