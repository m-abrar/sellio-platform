    ...
    <div class="container text-center">
        ...
        <div class="mb-4 mx-auto d-flex align-items-center justify-content-center rounded-3"
            ...
            @if($type->primary_image_url)
                <img src="{{ $type->primary_image_url }}" class="rounded-3 object-fit-cover w-100 h-100" alt="">
            @else
                <i class="bi bi-tag-fill fs-2" style="color:rgba(255,255,255,.8)"></i>
            @endif
        </div>
        ...
        <h1 class="page-hero-title">{{ $type->title }}</h1>

        @if(($type->listings_count ?? 0) > 0)
            <p class="page-hero-subtitle mx-auto">
                {{ number_format($type->listings_count) }} {{ __('listings in this type') }}
            </p>
        @endif
