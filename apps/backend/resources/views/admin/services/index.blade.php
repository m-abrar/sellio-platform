                            ...
                            <tr>
                                <td class="text-center align-middle">
                                    <div class="table-img-preview shadow-xs rounded-lg overflow-hidden border" style="width: 50px; height: 50px; margin: 0 auto;">
                                        <img src="{{ $service->thumbnail_url ?? asset('images/placeholder.png') }}" style="width: 100%; height: 100%; object-fit: cover;">
                                    </div>
                                </td>
                                    ...
                                    <div class="d-flex align-items-center">
                                        <div>
                                            ...
                                            <div class="d-flex align-items-center mt-1">
                                                <small class="badge badge-secondary-light mr-2" style="font-size: 0.65rem;">ID: {{ $service->id }}</small>
                                ...
                                <td class="align-middle small text-muted">
                                    ...
                                    {{ $service->city ?? 'Remote' }}{{ isset($service->country) ? ', ' . $service->country : '' }}
                                </td>

                                <td class="align-middle">
                                    <div class="font-weight-bold text-dark">{{ $service->price_formatted ?? '$0.00' }}</div>
                                    <small class="text-muted">Base Rate</small>
                                </td>

                                <td class="align-middle">
                                    @if($service->is_subscription)
                                        <span class="badge badge-premium badge-info-light">Subscription</span>
                                    @elseif($service->is_project_based)
                                        <span class="badge badge-premium badge-secondary-light">Project-Based</span>
                                    @else
                                        <span class="badge badge-premium badge-secondary-light">Fixed</span>
                                    @endif
                                </td>
                                ...
                                <td class="text-right align-middle px-4">
                                    <div class="btn-group btn-group-premium shadow-sm rounded-pill border overflow-hidden bg-white">
                                        <a href="{{ route('admin.services.edit', $service->id) }}" class="btn btn-white btn-sm text-info py-2 px-3 border-right" data-toggle="tooltip" title="Edit"><i class="fas fa-pencil-alt"></i></a>
                                        <a href="{{ route('admin.services.duplicate', $service->id) }}" class="btn btn-white btn-sm text-success py-2 px-3 border-right" data-toggle="tooltip" title="Duplicate"><i class="fas fa-copy"></i></a>
                                        <form action="{{ route('admin.services.destroy', $service->id) }}" method="POST" class="d-inline">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="btn btn-white btn-sm text-danger py-2 px-3" data-toggle="tooltip" title="Delete" onclick="return confirm('Permanently delete this service listing?')"><i class="fas fa-trash-alt"></i></button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="7" class="text-center py-5"><h5 class="text-muted">No Services Found</h5></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
