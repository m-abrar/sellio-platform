...
@section('title', ($advertisement->exists ? __('Edit') : __('Add')) . ' ' . __('Advertisement'))
    ...
    <div class="row mb-4 align-items-center">
        <div class="col-sm-8">
            <h1 class="m-0 text-dark font-weight-bold">
                ...
                {{ $advertisement->exists ? __('Edit Advertisement') : __('Create New Campaign') }}
            </h1>
            <p class="text-muted mt-2 small uppercase letter-spacing-1 mb-0">
                {{ $advertisement->exists ? __('Modify campaign parameters, creative assets, and targeting logic.') : __('Initialize a new marketing campaign with custom creative and placement rules.') }}
            </p>
        </div>
    ...
    <form id="ad-form" 
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                ...
                <div class="card border-0 shadow-premium mt-4 mb-4 rounded-xl overflow-hidden">
                    <div class="card-header border-0 bg-white py-4 px-4">
                        <h3 class="card-title-side">
                            <i class="fas fa-image mr-2 text-primary opacity-50"></i> {{ __('Creative Banner') }}
                        </h3>
                    </div>
                    <div class="card-body p-0">
                        @include('admin._partials._image-uploader', [
                            'name' => \App\Models\Advertisement::PRIMARY_MEDIA,
                            'label' => __('Select Ad Image'),
                            'multiple' => false,
                            'model' => 'advertisement',
                        ...
                        <div class="p-4 bg-light border-top">
                            <p class="text-muted mb-2 small uppercase letter-spacing-1"><strong>{{ __('Recommended Sizes:') }}</strong></p>
                            <ul class="list-unstyled small text-muted mb-0 leading-relaxed">
                                <li>• <strong>{{ __('General:') }}</strong> 600x300 (2:1)</li>
                                <li>• <strong>{{ __('Header/Footer:') }}</strong> 2000x100 (20:1)</li>
                                <li>• <strong>{{ __('Sidebar:') }}</strong> 300x600 (1:2)</li>
                            </ul>
                        </div>
                    </div>
                </div>
                ...
                <div class="card border-0 shadow-premium mt-4 mb-4 rounded-xl overflow-hidden">
                    <div class="card-header border-0 bg-white py-4 px-4">
                        <h3 class="card-title-side">{{ __('Placement Guide') }}</h3>
                    </div>
                    <div class="card-body p-3">
                        <div class="row no-gutters">
                            <div class="col-6 p-1 text-center">
                                ...
                                <small class="d-block mt-2 text-muted smallest uppercase letter-spacing-1">{{ __('Home Feed') }}</small>
                            </div>
                            <div class="col-6 p-1 text-center">
                                <img src="{{asset('admin-assets/diagram-search-page.png')}}" alt="Search" class="img-fluid border guide-img shadow-sm rounded-xl">
                                <small class="d-block mt-2 text-muted smallest uppercase letter-spacing-1">{{ __('Search Ops') }}</small>
                            </div>
                        </div>
                    </div>
                </div>
