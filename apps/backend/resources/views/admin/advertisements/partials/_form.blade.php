...
<div class="card border-0 shadow-premium rounded-xl overflow-hidden mb-4">
    <div class="card-header border-0 bg-white py-4 px-4">
        <h3 class="card-title-main">{{ __('Campaign Identity') }}</h3>
    </div>
    <div class="card-body p-4 pt-0">
        <div class="form-group mb-4">
            <label class="small font-weight-bold text-muted uppercase mb-2 letter-spacing-1">{{ __('Advertisement Title') }} <span class="text-danger">*</span></label>
            <input type="text" name="title" id="title" class="form-control form-control-hero @error('title') is-invalid @enderror" 
                   value="{{ old('title', $advertisement->title ?? '') }}" placeholder="{{ __('e.g. Summer Sale Banner') }}" required>
            @error('title') <span class="invalid-feedback d-block">{{ $message }}</span> @enderror
        </div>

        <div class="form-group mb-0">
            <label class="small font-weight-bold text-muted uppercase mb-2 letter-spacing-1">{{ __('Click-through URL (Link)') }}</label>
            <input type="url" name="link" id="link" class="form-control form-control-premium @error('link') is-invalid @enderror"
                   value="{{ old('link', $advertisement->link ?? '') }}" placeholder="{{ __('https://example.com/promo') }}">
...
<div class="card border-0 shadow-premium rounded-xl overflow-hidden mb-4">
    <div class="card-header border-0 bg-white py-4 px-4">
        <h3 class="card-title-main">{{ __('Geospatial Targeting') }}</h3>
    </div>
    <div class="card-body p-4 pt-0">
        <div class="row">
            <div class="col-md-6 form-group mb-4">
                <label class="small font-weight-bold text-muted uppercase mb-2 letter-spacing-1">{{ __('Latitude') }}</label>
            ...
            <div class="col-md-6 form-group mb-4">
                <label class="small font-weight-bold text-muted uppercase mb-2 letter-spacing-1">{{ __('Longitude') }}</label>
        ...
        <div class="form-group mb-4">
            <label class="small font-weight-bold text-muted uppercase mb-2 letter-spacing-1">{{ __('Specific Cities') }}</label>
            <input type="text" name="cities" id="cities" 
                class="form-control form-control-premium {{ $errors->has('cities') ? 'is-invalid' : '' }}" 
                placeholder="{{ __('e.g., New York, Los Angeles, Chicago') }}" 
        ...
        <div class="form-group mb-4">
            <label class="small font-weight-bold text-muted uppercase mb-2 letter-spacing-1">{{ __('Specific Zip Codes') }}</label>
            <input type="text" name="zipcodes" id="zipcodes" 
                class="form-control form-control-premium {{ $errors->has('zipcodes') ? 'is-invalid' : '' }}" 
                placeholder="{{ __('e.g., 10001, 90001, 60601') }}" 
        ...
        <div class="form-group mb-0">
            <label class="small font-weight-bold text-muted uppercase mb-2 letter-spacing-1">{{ __('Specific Regions') }}</label>
            <input type="text" name="regions" id="regions" 
                class="form-control form-control-premium {{ $errors->has('regions') ? 'is-invalid' : '' }}" 
                placeholder="{{ __('e.g., Northeast, Midwest, West') }}" 
...
<div class="card border-0 shadow-premium rounded-xl overflow-hidden mb-4">
    <div class="card-header border-0 bg-white py-4 px-4">
        <h3 class="card-title-main">{{ __('Placement Strategy') }}</h3>
    </div>
    <div class="card-body p-4 pt-0">
