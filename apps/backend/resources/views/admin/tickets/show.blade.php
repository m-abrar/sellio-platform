...
@section('title', __('Ticket Details #') . $ticket->id)
        ...
        <div class="row mb-4 align-items-center">
            <div class="col-sm-8">
                <h1 class="m-0 text-dark font-weight-bold">
                    <i class="fas fa-ticket-alt mr-2 text-primary"></i> {{ __('Support Manifest #') . $ticket->id }}
                </h1>
                <p class="text-muted mt-2 small text-uppercase letter-spacing-1 mb-0">{{ __('Unified thread for resolution tracking, stakeholder communication, and audit logging.') }}</p>
            </div>
            <div class="col-sm-4 text-right">
                <a href="{{ route('admin.tickets.index') }}" class="btn btn-back shadow-sm rounded-pill px-4 py-2 font-weight-bold smallest uppercase letter-spacing-1">
                    <i class="fas fa-arrow-left mr-1"></i> {{ __('Return to Queue') }}
                </a>
            </div>
        </div>
    </div>
            ...
            <div class="card border-0 shadow-premium overflow-hidden rounded-24">
                <div class="card-header border-0 bg-white py-4 px-4">
                    <h5 class="card-title font-weight-bold text-dark text-uppercase mb-0 ls-1-p">
                        <i class="fas fa-comments mr-2 text-primary opacity-50"></i> {{ __('Resolution Thread') }}
                    </h5>
                </div>
                        ...
                        <div class="d-flex justify-content-between align-items-start mb-3">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    ...
                                    <small class="text-muted smallest font-weight-bold uppercase">{{ $ticket->user->email ?? __('Direct Prospect') }}</small>
                                </div>
                            </div>
                            <span class="badge badge-white shadow-xs px-3 py-2 rounded-pill font-weight-bold smallest text-muted">
                                {{ __('INITIATED') }} {{ $ticket->created_at->format('M d, Y @ H:i') }}
                            </span>
                        </div>
                    ...
                    <div class="ticket-chat-thread mb-4 pr-2 h-max-600 overflow-y-auto">
                                ...
                                <div class="p-3 w-max-80-pct {{ $isAdmin ? 'bg-primary text-white shadow-premium rounded-bubble-admin' : 'bg-light border rounded-bubble-user' }}">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <span class="smallest font-weight-bold {{ $isAdmin ? 'text-white-50' : 'text-primary' }} uppercase letter-spacing-1">
                                            {{ $isAdmin ? __('System Agent') : ($message->user->name ?? __('User')) }}
                                        </span>
                        ...
                        @empty
                            <div class="text-center py-5 opacity-50">
                                ...
                                <p class="smallest font-weight-bold uppercase letter-spacing-1">{{ __('No active correspondence') }}</p>
                            </div>
                        @endforelse
                    </div>
                    ...
                    <div class="mt-5 border-top pt-4">
                        <form action="{{ route('admin.tickets.reply', $ticket->id) }}" method="POST">
                            ...
                            <div class="form-group mb-0">
                                <label class="text-uppercase smallest font-weight-bold letter-spacing-1 text-muted mb-2">{{ __('Internal Agency Response') }}</label>
                                <textarea name="body" class="form-control form-control-premium shadow-none border-light mb-3" rows="4" placeholder="{{ __('Draft your professional response...') }}" required></textarea>
                                <button type="submit" class="btn btn-primary rounded-pill px-5 font-weight-bold">
                                    <i class="fas fa-paper-plane mr-2"></i> {{ __('TRANSMIT REPLY') }}
                                </button>
                            </div>
                        </form>
                    </div>
            ...
            <div class="card border-0 shadow-premium overflow-hidden rounded-24">
                <div class="card-header border-0 bg-white py-4 px-4">
                    <h5 class="card-title font-weight-bold text-dark text-uppercase smallest mb-0 ls-1-p">
                        <i class="fas fa-shield-alt mr-2 text-primary opacity-50"></i> {{ __('Protocol & Meta') }}
                    </h5>
                </div>
                <div class="card-body p-0">
                    <div class="px-4 py-3 border-bottom d-flex justify-content-between align-items-center">
                        ...
                        @endphp
                        <span class="badge badge-{{ $statusColor }}-light text-{{ $statusColor }} px-3 py-1 rounded-pill font-weight-bold smallest">{{ strtoupper(__($ticket->status)) }}</span>
                    </div>
                    <div class="px-4 py-3 border-bottom d-flex justify-content-between align-items-center">
                        <span class="smallest font-weight-bold text-muted uppercase">{{ __('Urgency Tier') }}</span>
                        @php
                        ...
                        <span class="text-{{ $priorityColor }} font-weight-bold smallest uppercase">
                            <i class="fas fa-bolt mr-1"></i> {{ __($ticket->priority) }}
                        </span>
                    </div>
                    <div class="px-4 py-3 border-bottom d-flex justify-content-between align-items-center">
                        <span class="smallest font-weight-bold text-muted uppercase">{{ __('Account ID') }}</span>
                    ...
                    <div class="px-4 py-4 bg-light">
                        <label class="text-uppercase smallest font-weight-bold letter-spacing-1 text-muted mb-3 d-block text-center">{{ __('Transition Lifecycle State') }}</label>
                        <form action="{{ route('admin.tickets.status', $ticket->id) }}" method="POST">
                            ...
                            <div class="form-group mb-3">
                                <select name="status" class="form-control form-control-sm form-control-premium shadow-none border-light">
                                    <option value="open" @if($ticket->status === 'open') selected @endif>{{ __('Return to Open Queue') }}</option>
                                    <option value="in-progress" @if($ticket->status === 'in-progress') selected @endif>{{ __('Set to In-Progress') }}</option>
                                    <option value="closed" @if($ticket->status === 'closed') selected @endif>{{ __('Archive & Close Thread') }}</option>
                                    <option value="reopened" @if($ticket->status === 'reopened') selected @endif>{{ __('Re-Verify (Reopen)') }}</option>
                                </select>
                            </div>
                            <button type="submit" class="btn btn-white btn-sm btn-block rounded-pill border font-weight-bold uppercase smallest py-2">
                                {{ __('EXECUTE STATE CHANGE') }}
                            </button>
                        </form>
                    </div>
                </div>
                <div class="card-footer bg-white border-0 py-4 text-center">
                    <p class="smallest text-muted font-weight-bold uppercase mb-0">
                        <i class="fas fa-fingerprint mr-1 opacity-50"></i> {{ __('Manifest Secured & Audited') }}
                    </p>
                </div>
            </div>
        </div>
