        ...
        <div class="row mb-4 align-items-center">
            <div class="col-sm-8">
                <h1 class="m-0 text-dark font-weight-bold">
                    <i class="fas fa-file-invoice-dollar mr-2 text-primary opacity-50"></i> Payout Request #{{ $withdrawal->id }}
                </h1>
                <p class="text-muted mt-2 small text-uppercase letter-spacing-1 mb-0">
                    Review and synchronize financial settlement for marketplace partners.
                </p>
            </div>
...
<div class="container-fluid pb-5">
    ...
    <div class="row">
        ...
        <div class="col-md-8">
                ...
                <div class="col-md-6">
                    <div class="card border-0 shadow-premium rounded-xl bg-primary text-white overflow-hidden h-100">
                        <div class="card-body p-4 position-relative">
                            ...
                            <p class="smallest font-weight-bold mt-2 mb-0 text-white-50 uppercase">Base Units: {{ number_format($withdrawal->amount) }}</p>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="card border-0 shadow-premium rounded-xl h-100 bg-white">
                        <div class="card-body p-4 d-flex align-items-center">
                            ...
                            <div>
                                <h6 class="smallest font-weight-bold text-muted uppercase letter-spacing-2 mb-1">Request Status</h6>
            ...
            <div class="card border-0 shadow-premium rounded-xl overflow-hidden mb-4">
                <div class="card-header border-0 bg-white py-4 px-4">
                    <h3 class="card-title-main">Financial Intelligence</h3>
                </div>
                <div class="card-body p-4 pt-0">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group mb-4">
                                <label class="small font-weight-bold text-muted uppercase mb-2 d-block">Payout Protocol</label>
                        ...
                        <div class="col-md-6">
                            <div class="form-group mb-4">
                                <label class="small font-weight-bold text-muted uppercase mb-2 d-block">Temporal Signature</label>
                                <div class="p-3 bg-light rounded-xl border border-light-soft d-flex align-items-center">
            ...
            @if($withdrawal->status === 'pending')
                <div class="card border-0 shadow-premium rounded-xl overflow-hidden mb-4">
                    <div class="card-header border-0 bg-white py-4 px-4">
                        <h3 class="card-title-main">Administrative Actions</h3>
                    </div>
                    <div class="card-body p-4 pt-0">
                        <form action="{{ route('admin.withdrawals.approve', $withdrawal) }}" method="POST" id="payoutApproveForm">
                            @csrf
                            <div class="form-group mb-4">
                                <label class="small font-weight-bold text-muted uppercase mb-2 d-block">Internal Payout Note</label>
                            ...
                            <div class="d-flex gap-12">
                                <button type="button" class="btn btn-success rounded-pill px-4 py-3 flex-grow-1 font-weight-bold smallest uppercase letter-spacing-1 shadow-sm" onclick="triggerApproval()">
                                    <i class="fas fa-check-circle mr-2"></i> Approve & Process
                                </button>
            ...
            @elseif($withdrawal->admin_note)
                <div class="card border-0 shadow-premium rounded-xl overflow-hidden mb-4 border-left-premium-{{ $status['color'] }}">
                    <div class="card-header border-0 bg-white py-4 px-4">
                        <h3 class="card-title-main">Administrative Audit Log</h3>
                    </div>
                    <div class="card-body p-4 pt-0">
                        <div class="p-3 bg-{{ $status['color'] }}-soft text-{{ $status['color'] }} rounded-xl border border-{{ $status['color'] }}-light">
                            <i class="fas fa-info-circle mr-2"></i> <strong>Audit Note:</strong> {{ $withdrawal->admin_note }}
                        </div>
                    </div>
                </div>
            @endif
        ...
        <div class="col-md-4">
            ...
            <div class="card border-0 shadow-premium rounded-xl overflow-hidden mb-4">
                <div class="card-header border-0 bg-white py-4 px-4">
                    <h3 class="card-title-side">Partner Identity</h3>
                </div>
                <div class="card-body p-4 pt-0 text-center">
                    ...
                    <div class="border-top pt-4">
                        <div class="d-flex justify-content-between mb-2">
                            <span class="text-muted smallest font-weight-bold uppercase">Account ID</span>
                            <span class="text-dark font-weight-bold smallest">#{{ $withdrawal->user_id }}</span>
                        </div>
                        <div class="d-flex justify-content-between">
                            <span class="text-muted smallest font-weight-bold uppercase">Role</span>
                            <span class="badge badge-primary-light text-primary smallest rounded-pill px-2">PARTNER</span>
                        </div>
                    </div>
            ...
            <div class="card border-0 shadow-premium rounded-xl overflow-hidden mb-4 bg-light">
                <div class="card-body p-4">
                    <h6 class="font-weight-bold text-dark mb-3"><i class="fas fa-gavel mr-2 text-warning"></i> Settlement Policy</h6>
                    <ul class="list-unstyled smallest text-muted font-weight-600 leading-1-8">
                        <li><i class="fas fa-check text-success mr-2"></i> Manual verification required.</li>
                        <li><i class="fas fa-check text-success mr-2"></i> 2FA verification for admin.</li>
                        <li><i class="fas fa-check text-success mr-2"></i> Payout method must be active.</li>
                    </ul>
                </div>
            </div>
        </div>
