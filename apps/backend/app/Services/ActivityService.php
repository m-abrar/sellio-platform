        ...
        return \Illuminate\Support\Facades\Cache::remember($cacheKey, 300, function () use ($partner, $partnerId) {
            ...
            $recentActivityLog = $activities->sortByDesc('created_at')->take(15)->map(function ($activity) {
                if ($activity instanceof Review) {
                    return ['type' => 'Review', 'title' => 'New Review', 'description' => 'Received a new ' . $activity->rating . '-star review on "' . ($activity->reviewable->title ?? 'N/A') . '".', 'user' => $activity->user->name ?? 'Anon', 'time' => Carbon::parse($activity->created_at)->diffForHumans(), 'route' => '/dashboard/reviews', 'avatar' => $activity->user->avatar_url ?? null, 'image' => $activity->reviewable->primary_image_url ?? null];
                } elseif ($activity instanceof JobApplication) {
                    return ['type' => 'Application', 'title' => 'New Job Application', 'description' => 'New application received for "' . ($activity->job->title ?? 'N/A') . '".', 'user' => $activity->user->name ?? 'Applicant', 'time' => Carbon::parse($activity->created_at)->diffForHumans(), 'route' => '/dashboard/joblistings/applications/' . $activity->id, 'avatar' => $activity->user->avatar_url ?? null, 'image' => $activity->job->primary_image_url ?? null];
                } elseif ($activity instanceof PropertyBooking) {
                    return ['type' => 'Booking', 'title' => 'New Property Booking', 'description' => 'New booking request for "' . ($activity->property->title ?? 'N/A') . '".', 'user' => $activity->user->name ?? 'Guest', 'time' => Carbon::parse($activity->created_at)->diffForHumans(), 'route' => '/dashboard/properties/bookings/' . $activity->id, 'avatar' => $activity->user->avatar_url ?? null, 'image' => $activity->property->primary_image_url ?? null];
                } elseif ($activity instanceof EventBooking) {
                    return ['type' => 'Booking', 'title' => 'New Event Booking', 'description' => 'New booking for "' . ($activity->event->title ?? 'N/A') . '".', 'user' => $activity->user->name ?? 'Guest', 'time' => Carbon::parse($activity->created_at)->diffForHumans(), 'route' => '/dashboard/events/bookings/' . $activity->id, 'avatar' => $activity->user->avatar_url ?? null, 'image' => $activity->event->primary_image_url ?? null];
                } elseif ($activity instanceof ServiceQuote) {
                    return ['type' => 'Quote', 'title' => 'New Service Quote Request', 'description' => 'New quote request for "' . ($activity->service->title ?? 'N/A') . '".', 'user' => $activity->user->name ?? 'Customer', 'time' => Carbon::parse($activity->created_at)->diffForHumans(), 'route' => '/dashboard/services/quotes/' . $activity->id, 'avatar' => $activity->user->avatar_url ?? null, 'image' => $activity->service->primary_image_url ?? null];
                } elseif ($activity instanceof ServiceAppointment) {
                    return ['type' => 'Appointment', 'title' => 'New Service Appointment', 'description' => 'New appointment for "' . ($activity->service->title ?? 'N/A') . '".', 'user' => $activity->user->name ?? 'Customer', 'time' => Carbon::parse($activity->created_at)->diffForHumans(), 'route' => '/dashboard/services/appointments/' . $activity->id, 'avatar' => $activity->user->avatar_url ?? null, 'image' => $activity->service->primary_image_url ?? null];
                } elseif ($activity instanceof ClassifiedInquiry) {
                    $listingTitle = $activity->classifiedad->title ?? 'N/A';
                    return ['type' => 'Inquiry', 'title' => 'New Listing Inquiry', 'description' => 'New inquiry for "' . $listingTitle . '".', 'user' => $activity->user->name ?? 'Buyer', 'time' => Carbon::parse($activity->created_at)->diffForHumans(), 'route' => '/dashboard/classifieds/inquiries/' . $activity->id, 'avatar' => $activity->user->avatar_url ?? null, 'image' => $activity->classifiedad->primary_image_url ?? null];
                } elseif ($activity instanceof AutoInquiry) {
                    $listingTitle = $activity->auto->title ?? 'N/A';
                    return ['type' => 'Inquiry', 'title' => 'New Auto Inquiry', 'description' => 'New inquiry for "' . $listingTitle . '".', 'user' => $activity->user->name ?? 'Buyer', 'time' => Carbon::parse($activity->created_at)->diffForHumans(), 'route' => '/dashboard/autos/inquiries/' . $activity->id, 'avatar' => $activity->user->avatar_url ?? null, 'image' => $activity->auto->primary_image_url ?? null];
                } elseif ($activity instanceof Message) {
                    return ['type' => 'Message', 'title' => 'New Message', 'description' => 'New message from ' . ($activity->sender->name ?? 'Guest') . '.', 'user' => $activity->sender->name ?? 'Guest', 'time' => Carbon::parse($activity->created_at)->diffForHumans(), 'route' => '/dashboard/messages', 'avatar' => $activity->sender->avatar_url ?? null, 'image' => null];
                }
                return null;
            })->filter()->values();
