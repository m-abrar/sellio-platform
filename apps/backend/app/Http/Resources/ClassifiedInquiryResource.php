...
class ClassifiedInquiryResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        $classifiedData = $this->whenLoaded('classifiedAd', function () {
            if (! $this->classifiedAd) {
                return null;
            }
            ...
            return [
                ...
                'brand' => $this->classifiedAd->relationLoaded('brand') && $this->classifiedAd->brand ? [
        ...
        return [
            ...
            'full_name' => $this->when($canViewPii, $this->name ?? ($this->relationLoaded('user') ? $this->user?->name : null)),
            'email' => $this->when($canViewPii, $this->email ?? ($this->relationLoaded('user') ? $this->user?->email : null)),
            'phone' => $this->when($canViewPii, $this->phone ?? ($this->relationLoaded('user') ? $this->user?->phone : null)),
