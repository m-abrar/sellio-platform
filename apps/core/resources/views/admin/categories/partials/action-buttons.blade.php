<div class="card shadow-sm border-0 overflow-hidden sticky-top" style="top: 20px;">
    <div class="card-header bg-dark d-flex align-items-center py-3" style="border-bottom: 3px solid var(--brand-primary) !important;">
        <h3 class="card-title text-white mb-0 font-weight-bold">
            <i class="fas fa-cog mr-2 text-primary"></i> Publishing Settings
        </h3>
    </div>
    ...
    <div class="card-body bg-white py-4">
        ...
        <div class="mb-4 pb-2 border-bottom">
            <label class="w-100 cursor-pointer mb-0">
                ...
                <input type="checkbox" name="is_published" value="1" id="publishedSwitch" class="d-none toggle-input" {{ old('is_published', $category->is_published ?? '0') == '1' ? 'checked' : '' }}>
                <div class="border rounded px-3 py-2 d-flex justify-content-between align-items-center toggle-card shadow-sm">
                    <div>
                        ...
                        <div class="small toggle-status text-muted">{{ (isset($category) && $category->is_published) ? 'Visible to public' : 'Draft Mode' }}</div>
                    </div>
        ...
        <button type="submit" class="btn btn-primary btn-block btn-lg btn-flat shadow-sm font-weight-bold">
            <i class="fas fa-save mr-2"></i> {{ isset($category) ? 'UPDATE CATEGORY' : 'CREATE CATEGORY' }}
        </button>

        @if(isset($category))
            <div class="row gx-1 mt-2">
                <div class="col-6">
                    @if(Route::has('admin.categories.duplicate'))
                        <a href="{{ route('admin.categories.duplicate', $category->id) }}" class="btn btn-default btn-block btn-flat btn-sm text-secondary"><i class="fas fa-copy mr-1"></i> Duplicate</a>
                    @else
    ...
    @if(isset($category))
        <div class="card-footer bg-light border-top-0 text-center">
            <small class="text-muted">
                ...
                Last Sync: {{ $category->updated_at->format('M d, Y H:i') }}
            </small>
        </div>
    @endif
</div>
