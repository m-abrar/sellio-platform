<div class="card shadow-sm border-0 overflow-hidden sticky-top" style="top: 20px;">
    ...
    <div class="card-body bg-white py-4">
            ...
            <label class="w-100 cursor-pointer mb-0">
                ...
                <div class="border rounded px-3 py-2 d-flex justify-content-between align-items-center toggle-card shadow-sm">
                    <div>
                        ...
                        <div class="small toggle-status text-muted">{{ (isset($brand) && $brand->is_published) ? 'Visible to public' : 'Draft Mode' }}</div>
                    </div>
        ...
        <button type="submit" class="btn btn-primary btn-block btn-lg btn-flat shadow-sm font-weight-bold">
            <i class="fas fa-save mr-2"></i> {{ isset($brand) ? 'UPDATE BRAND' : 'CREATE BRAND' }}
        </button>
            ...
            <div class="row gx-1 mt-2">
                <div class="col-6">
                    @if(Route::has('admin.brands.duplicate'))
                        <a href="{{ route('admin.brands.duplicate', $brand->id) }}" class="btn btn-default btn-block btn-flat btn-sm text-secondary"><i class="fas fa-copy mr-1"></i> Duplicate</a>
                    @else
