    ...
    <div class="card-body bg-white py-4">
        ...
        <div class="mb-4 pb-2 border-bottom">
            <label class="w-100 cursor-pointer mb-0">
                ...
                <input type="checkbox" name="is_published" value="1" id="publishedSwitch" class="d-none toggle-input" {{ old('is_published', $feature->is_published ?? '0') == '1' ? 'checked' : '' }}>
                <div class="border rounded px-3 py-2 d-flex justify-content-between align-items-center toggle-card shadow-sm">
                    <div>
                        ...
                        <div class="small toggle-status text-muted">{{ (isset($feature) && $feature->is_published) ? 'Visible to public' : 'Draft Mode' }}</div>
                    </div>
            ...
            <div class="row gx-1 mt-2">
                <div class="col-6">
                    @if(Route::has('admin.features.duplicate'))
                        <a href="{{ route('admin.features.duplicate', $feature->id) }}" class="btn btn-default btn-block btn-flat btn-sm text-secondary"><i class="fas fa-copy mr-1"></i> Duplicate</a>
                    @else
    ...
    @if(isset($feature))
        <div class="card-footer bg-light border-top-0 text-center">
            <small class="text-muted">
                ...
                Last Sync: {{ $feature->updated_at->format('M d, Y H:i') }}
            </small>
        </div>
    @endif
</div>
