<div class="card shadow-sm border-0 sticky-top" style="top: 20px;">
    ...
    <div class="card-body">
            ...
            <label class="w-100 cursor-pointer mb-0">
                ...
                <div class="border rounded px-3 py-2 d-flex justify-content-between align-items-center toggle-card shadow-sm">
                    <div>
                        ...
                        <div class="small toggle-status text-muted">{{ (isset($job) && $job->is_published) ? 'Visible to public' : 'Draft Mode' }}</div>
                    </div>
            ...
            <label class="w-100 cursor-pointer mb-0">
                ...
                <div class="border rounded px-3 py-2 d-flex justify-content-between align-items-center toggle-card shadow-sm">
                    <div>
                        ...
                        <div class="small toggle-status text-muted">{{ (isset($job) && $job->is_featured) ? 'Featured' : 'Standard' }}</div>
                    </div>
                    <div class="toggle-indicator"></div>
                </div>
            </label>
        ...
        <button type="submit" class="btn btn-primary btn-block btn-lg btn-flat shadow-sm font-weight-bold mb-2">
            <i class="fas fa-save mr-2"></i> {{ isset($job) ? 'UPDATE JOB' : 'CREATE JOB' }}
        </button>
            ...
            <div class="row gx-1">
                <div class="col-6">
                    @if(Route::has('admin.jobs.duplicate'))
                        <a href="{{ route('admin.jobs.duplicate', $job->id) }}" class="btn btn-default btn-block btn-flat btn-sm text-secondary"><i class="fas fa-copy mr-1"></i> Duplicate</a>
                    @else
