...
export function CatalogSyncAlert({
  ...
  return (
    <div className={`${prefix}-catalog-alert ${prefix}-catalog-alert--prod`} role="status">
      ...
      <h3 className={`${prefix}-catalog-alert__title`}>Services unavailable</h3>
      <p className={`${prefix}-catalog-alert__copy`}>
        Services could not be loaded. Check your API connection and make sure listings are published in the admin.
      </p>
    </div>
  );
}
