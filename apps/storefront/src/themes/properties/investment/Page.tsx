  ...
  return (
    <div className="pi-section">
      ...
      <section className="pi-hero">
        <div>
          <div className="pi-mono" style={{ marginBottom: '2.5rem' }}>{useThemeContent('hero.kicker', 'Investment Portfolio')}</div>
          <h1 className="pi-heading-xl">
            {useThemeContent('hero.title', 'Capital \nPerformance \nVerified.').split('\n').map((line, i, arr) => {
              const highlight = useThemeContent('hero.highlight', 'Verified.');
          ...
          <p style={{ marginTop: '4rem', fontSize: '1.25rem', color: 'var(--pi-slate)', lineHeight: 1.8, maxWidth: '600px' }}>
            {useThemeContent('hero.description', 'Discover verified real estate investment opportunities with data-driven yield analysis and global portfolio coverage.')}
          </p>
          <div style={{ marginTop: '6rem', display: 'flex', gap: '3rem' }}>
            <a href={themeLink('/explore')} className="pi-btn-primary" style={{ textDecoration: 'none' }}>{useThemeContent('hero.primary_cta_label', 'Explore Properties')}</a>
            <a href={themeLink('/explore')} style={{ background: 'transparent', border: '2px solid var(--pi-midnight)', color: 'var(--pi-midnight)', padding: '1.5rem 4rem', fontWeight: 800, textTransform: 'uppercase', cursor: 'pointer', textDecoration: 'none' }}>{useThemeContent('hero.secondary_cta_label', 'View Reports')}</a>
          </div>
        </div>
        <div className="pi-hero-terminal">
          <div className="pi-mono" style={{ marginBottom: '3rem', borderBottom: '1px solid var(--pi-border)', paddingBottom: '1.5rem' }}>{useThemeContent('terminal.metrics_label', 'Portfolio Metrics')}</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem' }}>
            <YieldAnalyticsHUD label={useThemeContent('hud.total_volume_label', 'Total Volume')} value={useThemeContent('hud.total_volume_value', '$4.2B+')} color="var(--pi-emerald)" />
            <YieldAnalyticsHUD label={useThemeContent('hud.avg_yield_label', 'Average Yield')} value={useThemeContent('hud.avg_yield_value', '8.4%')} />
            <YieldAnalyticsHUD label={useThemeContent('hud.liquidity_label', 'Liquidity Index')} value={useThemeContent('hud.liquidity_value', '0.82')} />
            <YieldAnalyticsHUD label={useThemeContent('hud.volatility_label', 'Volatility Hedge')} value={useThemeContent('hud.volatility_value', 'Low')} color="var(--pi-gold)" />
          </div>
        </div>
      </section>
      ...
      <div style={{ padding: '3rem 6%', background: 'white', borderBottom: '1px solid var(--pi-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          {useThemeContent('logic.items', 'Market Status: Stable|Verification: 100%|Auth Status: Verified|Settlement: Instant').split('|').map(logic => (
      ...
      <section style={{ marginTop: '10rem' }}>
          ...
          <div className="pi-asset-grid">
            {loadingProperties ? (
              ...
              <div className="prop-listing-state">
                <div className="prop-listing-kicker">{useThemeContent('empty.kicker', 'No Listings Yet')}</div>
                <h3>{useThemeContent('empty.title', 'No properties are published yet.')}</h3>
      ...
      <section style={{ marginTop: '15rem', padding: '12rem 8%', background: 'var(--pi-midnight)', color: 'white', borderRadius: '4px', textAlign: 'center' }}>
          <div className="pi-mono" style={{ color: 'var(--pi-emerald)', marginBottom: '3rem' }}>{useThemeContent('cta.kicker', 'Get Started')}</div>
          ...
          <p style={{ maxWidth: '800px', margin: '0 auto 8rem', opacity: 0.5, fontSize: '1.25rem', lineHeight: 1.8 }}>
              {useThemeContent('cta.description', 'Join thousands of investors accessing verified real estate opportunities on the Sellio network.')}
          </p>
          <a href={themeLink('/explore')} className="pi-btn-primary" style={{ background: 'var(--pi-emerald)', padding: '2.5rem 8rem', fontSize: '1.25rem', textDecoration: 'none' }}>
              {useThemeContent('cta.button_label', 'Explore Opportunities')}
          </a>
      </section>
