...
export function usePropertyBookingPayment(
  bookingId: number,
  ...
  const [context, setContext] = useState<PropertyBookingPaymentContext | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadContext() {
      ...
      try {
        const paymentContext = await api.getPropertyBookingPaymentContext(bookingId);
  ...
  const handlePaymentSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const result = await api.payPropertyBooking(bookingId, paymentMethod, {
