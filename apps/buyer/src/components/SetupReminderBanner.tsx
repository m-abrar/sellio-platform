    ...
    >
      <div className="mx-auto flex max-w-5xl flex-col gap-3 sm:flex-row sm:items-start sm:gap-4">
        ...
        <div className="min-w-0 flex-1 space-y-2">
          ...
          <p className="text-sm font-extrabold text-amber-950">{primary.title}</p>
          <p className="text-sm text-amber-900/90">{primary.detail}</p>

          <ol className="list-decimal space-y-1 pl-5 text-sm text-amber-900/90">
            <li>Open <code className="rounded bg-amber-100 px-1.5 py-0.5 font-mono text-xs">config.js</code> in your buyer subdomain folder (cPanel File Manager or FTP).</li>
          ...
          <p className="flex items-center gap-2 text-xs font-bold text-amber-800/90">
            ...
            Also set <code className="font-mono">BUYER_APP_URL</code> in Laravel <code className="font-mono">.env</code> for CORS.
          </p>
        </div>
      </div>
    </div>
