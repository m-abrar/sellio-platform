...
function inspectUrl(
  ...
  if (!url?.trim()) {
    return [{
      ...
      detail: 'Open config.js in your buyer subdomain root (next to index.html) and add the required URL.',
    }];
  }
  ...
  if (isPlaceholderHost(host)) {
    return [{
      ...
      detail: `Replace the placeholder with your live domain (currently "${url.trim()}").`,
    }];
  }
...
export function getPortalSetupIssues(): PortalSetupIssue[] {
  ...
  const issues = [
    ...inspectUrl(config?.apiUrl, 'API URL'),
    ...inspectUrl(config?.storefrontUrl, 'Storefront URL'),
  ];
