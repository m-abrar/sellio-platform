...
export default function ListingActivityWidget({ listingId, listingType }: ListingActivityWidgetProps) {
  ...
  if (isLoading) {
    return (
      <div className="bg-white border border-slate-100 rounded-container p-8 shadow-sm flex flex-col items-center justify-center min-h-[200px]">
        <span className="text-micro font-black uppercase tracking-caps-wide text-slate-300 animate-pulse">Syncing Activities...</span>
      </div>
    );
  }
  ...
  if (activities.length === 0) {
    return (
      <div className={containerClass}>
        <h4 className="text-label font-black text-slate-400 uppercase tracking-label-caps mb-6 flex items-center gap-2">
          <HiOutlineClipboardDocumentList className="w-4 h-4 stroke-[2.5px]" /> Recent Activity
        </h4>
        <div className="text-center py-6">
          <p className="text-label font-black text-slate-400 uppercase tracking-widest">No Recent Bookings or Inquiries</p>
          <p className="text-micro font-bold text-slate-400 mt-2 leading-relaxed max-w-[220px] mx-auto">
  ...
  return (
    <div className={`${containerClass} relative overflow-hidden group animate-in fade-in zoom-in-95 duration-500`}>
      <h4 className="text-label font-black text-brand uppercase tracking-label-caps mb-8 flex items-center justify-between gap-3 flex-wrap">
        <span className="flex items-center gap-2">
          <HiOutlineClipboardDocumentList className="w-4 h-4 stroke-[2.5px]" /> Related Bookings & Inquiries
        </span>
        <span className="bg-brand/10 px-3 py-1 rounded-full text-micro font-extrabold text-brand tracking-normal">
        ...
        {activities.map((activity) => {
          ...
          return (
            <div 
              ...
              className="group/item flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 bg-white hover:bg-slate-50/50 border border-slate-100 hover:border-brand/20 rounded-2xl cursor-pointer transition-all duration-300 hover:scale-[1.01] hover:shadow-card"
            >
              <div className="flex items-center gap-4 min-w-0 w-full sm:w-auto">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-xs font-black text-slate-800 truncate leading-snug mb-1 group-hover/item:text-brand transition-colors">{activity.customer}</p>
                  <div className="flex items-center gap-x-2 gap-y-1 flex-wrap leading-none">
                    <span className="text-tiny font-black uppercase tracking-wider text-slate-400 bg-slate-50 px-1.5 py-0.5 rounded-md border border-slate-100/50 leading-none">
                      {activityMeta.label}
                    </span>
                    <span className="text-tiny font-bold text-slate-400 shrink-0 flex items-center gap-1 leading-none">
              ...
              <div className="flex items-center justify-between sm:justify-end gap-3.5 shrink-0 w-full sm:w-auto border-t border-slate-50 sm:border-none pt-3.5 sm:pt-0">
                <div className="flex flex-col items-start sm:items-end gap-1.5 leading-none">
                  {activity.amount && activity.amount !== '—' && (
                    <p className="text-xs font-black text-brand italic leading-none tracking-tight">{activity.amount}</p>
                  )}
                ...
                <div className="w-8 h-8 bg-slate-50 border border-slate-100/80 rounded-lg flex items-center justify-center text-slate-300 group-hover/item:text-brand group-hover/item:bg-purple-50 group-hover/item:border-purple-100/50 transition-all duration-300">
