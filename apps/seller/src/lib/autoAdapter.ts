...
export const normalizeAuto = (auto: any): NormalizedAuto => {
  ...
  return {
    ...
    vin_number: auto.specs?.vin ?? auto.specs?.raw_vin,
    ...
    condition_rating: auto.specs?.condition?.replace('/10', ''),
    warranty_months: auto.specs?.warranty?.replace(' Months', ''),
    stock_quantity: auto.stock_quantity ?? 1,
    ...
    is_published: auto.status?.is_published ?? false,
    is_featured: auto.status?.is_featured ?? false,
    is_lease: auto.pricing?.is_lease ?? false,
    is_selling: auto.pricing?.is_selling ?? true,
