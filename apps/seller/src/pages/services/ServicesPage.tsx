        ...
        <>
          <div className="lg:hidden space-y-6">
            {services.map((service) => (
              <div key={service.id} className="bg-white p-8 rounded-container border border-slate-100 shadow-premium relative overflow-hidden group">
                <div className="flex gap-6">
                  <div className="w-24 h-24 rounded-card-lg overflow-hidden bg-slate-50 shrink-0 border-4 border-white shadow-md cursor-pointer" onClick={() => navigate(`/dashboard/services/view/${service.slug}`)}>
                    <img src={service.media[0]?.original_url} className="w-full h-full object-cover" alt={service.title} loading="lazy" />
                  </div>
                  <div className="min-w-0 flex-1 pt-1">
                    <span className="text-micro font-black text-brand bg-brand/5 px-3 py-1 rounded-full uppercase tracking-widest">{service.sku}</span>
                    <h3 
                ...
                <div className="flex items-center justify-between mt-6 pt-6 border-t border-slate-50">
                  <div className="flex flex-col text-left">
                    <span className="text-micro font-black text-slate-300 uppercase tracking-widest mb-1">Coverage</span>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => handleDelete(service.id, service.title)} className="p-4 bg-red-50/50 text-red-400 rounded-2xl hover:bg-red-500 hover:text-white transition-all"><HiOutlineTrash className="w-5 h-5" /></button>
                  </div>
                </div>
              </div>
            ))}
            ...
            <table className="w-full border-separate border-spacing-y-4">
              <thead>
                <tr className="text-left text-caption font-black uppercase tracking-caps-wide text-slate-400">
                  <th className="px-10 pb-2">Service Identity</th>
                  <th className="px-10 pb-2">Rate Base</th>
