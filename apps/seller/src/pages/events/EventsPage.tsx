    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      {isLoading ? (
        <div className="h-64 flex items-center justify-center">
          <span className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-300 animate-pulse">Syncing Calendar...</span>
        </div>
      ) : (
        <>
          {/* MOBILE VIEW */}
              ...
              <div key={event.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-premium relative overflow-hidden group">
                <div className="flex gap-6">
                  ...
                  <div className="min-w-0 flex-1 pt-1">
                    ...
                    <p className="text-2xl font-black text-slate-900 mt-1 tracking-tighter">{event.price}</p>
                  </div>
                </div>
                ...
                {events.map((event) => (
                  <tr key={event.id} className="group">
                    <td className="bg-white group-hover:bg-slate-50/50 border-y border-l border-slate-100 group-hover:border-[#6610f2]/20 rounded-l-[2rem] px-10 py-6 transition-all duration-300">
                      <div className="flex items-center gap-6">
                        ...
                        <div className="min-w-0">
                          ...
                          <span className="text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest bg-[#6610f2]/5 text-[#6610f2] border border-[#6610f2]/10">{event.sku}</span>
                        </div>
                      </div>
                    </td>
                    <td className="bg-white group-hover:bg-slate-50/50 border-y border-slate-100 group-hover:border-[#6610f2]/20 px-10 py-6 transition-all duration-300">
                      <span className="text-xl font-black text-slate-900 tracking-tighter">{event.price}</span>
