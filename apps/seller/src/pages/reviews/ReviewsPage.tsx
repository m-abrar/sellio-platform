  ...
  if (isLoading) {
    return (
      <div className="h-screen flex items-center justify-center">
        <span className="text-label font-black uppercase tracking-caps-xl text-slate-300 animate-pulse">Syncing Feedback...</span>
      </div>
    );
  }
    ...
    <div className="space-y-10 md:space-y-16 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      <div className="bg-white p-8 md:p-12 rounded-container border border-slate-100 shadow-elite-soft flex flex-col md:flex-row items-stretch gap-10 md:gap-14">
        ...
        <div className="flex flex-col justify-center items-center text-center md:border-r border-slate-100 pr-0 md:pr-14 shrink-0">
          ...
          <p className="text-label font-black text-slate-400 uppercase tracking-widest leading-none">
          ...
          {ratingSpread.counts.map((count, idx) => {
            ...
            return (
              <div key={idx} className="flex items-center gap-4 text-xs font-bold text-slate-500">
                <span className="w-12 shrink-0 text-right uppercase text-micro font-black tracking-widest">{stars} Star</span>
                ...
                <span className="w-10 shrink-0 text-label font-black text-slate-700 tracking-tighter">
      ...
      <div className="space-y-8">
        {filteredReviews.length === 0 ? (
          <div className="bg-white border border-slate-100 rounded-container p-20 text-center shadow-xs">
            <span className="text-label font-black uppercase tracking-caps-xl text-slate-300">
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-8 border-b border-slate-50 pb-6">
                  ...
                  <div className="flex items-center gap-5">
                    {review.avatar_url ? (
                      <img
                        ...
                        className="w-14 h-14 rounded-inner object-cover border border-slate-100 shadow-sm shrink-0"
                        alt="avatar"
                      />
                    ) : (
                      <div className="w-14 h-14 rounded-inner bg-slate-50 flex items-center justify-center font-black text-slate-900 border border-slate-100 shrink-0 text-xl italic shadow-inner">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-2.5 mt-2.5 leading-none">
                        <span className={`text-micro font-black border px-3 py-1 rounded-full uppercase tracking-wider ${reviewMeta.bg} ${reviewMeta.border}`} style={{ color: reviewMeta.color }}>
                          {reviewMeta.icon} {review.type === 'JobListing' ? 'Job' : review.type}
                        </span>
                        <span className="text-micro font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                          On: 
                          {review.asset_image ? (
                  ...
                  <div className="flex items-center gap-5 shrink-0 w-full sm:w-auto justify-between sm:justify-end border-t sm:border-t-0 pt-4 sm:pt-0 border-slate-50">
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2 border px-5 py-3 rounded-full text-micro font-black uppercase tracking-widest shadow-xs transition-all duration-300 group cursor-pointer ${
                        review.isFeatured
                ...
                <div className="space-y-6">
                  ...
                  {review.partnerReply && (
                    <div className="p-6 bg-slate-950 text-white rounded-card shadow-xl shadow-slate-900/5 relative overflow-hidden group/reply transition-all duration-300">
                      <div className="relative z-10 flex flex-col gap-2 min-w-0">
                        <div className="flex items-baseline gap-2 min-w-0">
                          <p className="text-tiny font-black uppercase tracking-label-caps text-slate-400 mb-1 leading-none shrink-0">Partner Response</p>
                      ...
                      <div className="absolute -top-10 -right-10 w-24 h-24 bg-brand/10 rounded-full blur-2xl group-hover/reply:bg-brand/20 transition-all duration-500" />
                    </div>
                  )}
                  ...
                  <div className="pt-6 border-t border-slate-50 space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-micro font-bold text-slate-400 uppercase tracking-widest">{review.date}</span>
                      <span className="text-micro font-black uppercase tracking-widest text-brand bg-purple-50 px-4 py-1.5 rounded-full border border-purple-100/50 flex items-center gap-1.5 shrink-0">
                    ...
                    <button
                      ...
                      className="flex items-center justify-center bg-slate-900 hover:bg-slate-800 text-white hover:shadow-lg hover:shadow-slate-100/50 py-4 px-8 rounded-full text-label font-black uppercase tracking-widest transition-all duration-300 disabled:opacity-50 cursor-pointer active:scale-95 shrink-0"
                    >
      ...
      {pagination && pagination.lastPage > 1 && (
        <div className="flex justify-between items-center bg-white border border-slate-100 p-5 rounded-card shadow-elite-soft select-none">
          <button
            onClick={() => fetchReviews(currentPage - 1)}
            disabled={currentPage === 1 || isLoading}
            className="px-6 py-3 rounded-full border border-slate-100 text-label font-black uppercase tracking-widest text-slate-500 hover:text-slate-900 bg-white hover:bg-slate-50 active:scale-95 disabled:opacity-50 transition-all cursor-pointer"
          >
            Previous
          </button>
          
          <span className="text-label font-black uppercase tracking-widest text-slate-400">
