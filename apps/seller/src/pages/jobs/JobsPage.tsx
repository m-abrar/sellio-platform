      ...
      {isLoading ? (
        ...
        <>
          <div className="lg:hidden space-y-6">
            {jobs.map((job) => (
              <div key={job.id} className="bg-white p-8 rounded-container border border-slate-100 shadow-premium relative overflow-hidden group">
                <div className="flex gap-6">
                  <div className="w-24 h-24 rounded-card-lg overflow-hidden bg-slate-50 shrink-0 border-4 border-white shadow-md cursor-pointer" onClick={() => navigate(`/dashboard/joblistings/view/${job.slug}`)}>
                    <img src={job.media[0]?.original_url} className="w-full h-full object-cover" alt={job.title} loading="lazy" />
                  </div>
                  <div className="min-w-0 flex-1 pt-1">
                    <span className="text-micro font-black text-brand bg-brand/5 px-3 py-1 rounded-full uppercase tracking-widest">{job.sku}</span>
                    <h3 
