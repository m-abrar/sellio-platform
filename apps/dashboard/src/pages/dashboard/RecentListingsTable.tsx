...
const MobileActionBtn = ({ Icon, label, color }: any) => (
  <button className={`flex items-center justify-center gap-3 py-4 w-full rounded-[1.5rem] bg-white border border-slate-100 text-slate-400 transition-all active:scale-[0.96] active:text-white shadow-sm group ${color}`}>
        ...
        {listings.map((item) => (
          <div key={item.id} className="bg-slate-50/50 border border-slate-100 rounded-[2.5rem] p-8 shadow-sm">
            ...
            <div className="grid grid-cols-3 gap-3 mt-8 pt-8 border-t border-slate-200/50">
              <MobileActionBtn Icon={HiOutlineEye} label="View" color="active:bg-blue-600" />
              <MobileActionBtn Icon={HiOutlinePencilSquare} label="Edit" color="active:bg-[#6610f2]" />
          ...
          <tbody>
            {listings.map((item) => (
              <tr key={item.id} className="group">
                {/* ASSET IDENTITY */}
                <td className="bg-slate-50/40 group-hover:bg-white border-y border-l border-slate-100 group-hover:border-[#6610f2]/20 rounded-l-[1.5rem] px-10 py-8 transition-all duration-300">
                ...
                <td className="bg-slate-50/40 group-hover:bg-white border-y border-r border-slate-100 group-hover:border-[#6610f2]/20 rounded-r-[1.5rem] px-10 py-8 text-right transition-all duration-300 relative overflow-hidden">
                  <div className="relative h-12 flex items-center justify-end">
                    ...
                    <div className="absolute inset-y-0 right-0 flex items-center gap-3 opacity-0 translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300">
                      {[
                        { Icon: HiOutlineEye, bg: 'hover:bg-blue-600' },
                        { Icon: HiOutlinePencilSquare, bg: 'hover:bg-[#6610f2]' },
                        { Icon: HiOutlineTrash, bg: 'hover:bg-red-500' }
                      ].map((btn, idx) => (
                        <button 
                          key={idx} 
                          className={`p-4 text-slate-400 bg-white rounded-2xl border border-slate-100 hover:text-white shadow-sm transition-all active:scale-90 ${btn.bg}`}
                        >
